...
class Vectorizer:
    ...
    def __init__(self):
        ...
        self.partitions = []
        self.tag2tag = dict()
        # type token,tags ,pos_tok, pos_tag -> dict()
    ...
    def fit(self, tokens, tags):
        ...
        summary = """Vectorizer Fitted:
            ...
            number of sentences: {}
            """.format(
            len(self.feature_list), self.vector_size, len(tokens)
        )
        print(summary)
    ...
    def build_tag_map(self, tag_seq):
        ...
        for seq in tag_seq:
            ...
            for tag, next_tag in zip(seq, seq[1:]):
                ...
                # if in t2t
                if self.tag2tag.get(next_tag) is not None:
    ...
    def feature(self, token_seq, tag, prev_tag, pos):
        tag_seq = tuple([prev_tag, tag])
        curr_tok = token_seq[pos]

        if self.memoization_table.get((curr_tok, tag_seq)) is not None:

            return self.memoization_table[curr_tok, tag_seq]
        else:
            res = self.join_features(token_seq, tag_seq, pos, 1)
            self.memoization_table[curr_tok, tag_seq] = res
            return res
    ...
    def sparse_feat_word_in(self, token_seq, pos, off_set=0):
        token = token_seq[pos]
        if self.word_map.get(token) is not None:
    ...
    def sparse_feat_word_and_tag(self, token_seq, tag_seq, pos_tok, pos_tag, off_set=0):
        ...
        if (self.word_map.get(tok) is not None) and (self.tag_map.get(tag) is not None):
    ...
    def sparse_feat_prev_tag(self, token_seq, tag_seq, pos_tok, pos_tag, off_set=0):
        ...
        if self.tag_map.get(tag) is not None and self.tag_map.get(prev_tag) is not None:
    ...
    def get_trans_idx(self, token_seq, tag, prev, pos, weights):
        ...
        idx, val = self.sparse_feat_prev_tag(
            token_seq, tag_seq, pos, 1, off_set=beg_trans
        )
...
def main():
    ...
    print(v1, v2)
    print(vectorizer.sum_features(v1, v2))
